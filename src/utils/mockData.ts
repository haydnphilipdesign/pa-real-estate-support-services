...
export const generateMockFormData = (): TransactionFormData => {
  ...
  const role: AgentRole = randomPick(["Buyer's Agent", "Listing Agent", "Dual Agent"]);
  ...
  return {
    ...
    commissionBase: randomPick<CommissionBase>(["Sale Price", "Net Proceeds (After Seller's Assistance)"]),
    totalCommission,
