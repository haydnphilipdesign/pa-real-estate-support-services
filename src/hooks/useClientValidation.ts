...
import { TransactionFormData, ValidationState } from '../components/TransactionForm/types';
...
export function useClientValidation() {
  const validateEmail = useCallback((email: string): ValidationResult => {
    if (!email) {
      return { isValid: false, state: 'invalid', error: 'Email is required' };
    }
  ...
  const validatePhone = useCallback((phone: string): ValidationResult => {
    if (!phone) {
      return { isValid: false, state: 'invalid', error: 'Phone number is required' };
    }
